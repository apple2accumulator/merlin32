...
int BuildAllCodeLineSize(struct omf_segment *current_omfsegment)
{
    ...
    /*** Process all lines pour calculer leur taille ***/
...
int BuildAllCodeLine(int *has_error_rtn, struct omf_segment *current_omfsegment, struct omf_project *current_omfproject)
{
    ...
    /*** Process all lines pour créer le code binaire de l'Operand ***/
    for(current_line=first_file->first_line; current_line; current_line=current_line->next)
    {
        /* Cette ligne n'est pas valide */
        ...
        if(current_line->type == LINE_CODE)
        {
            /** Create the code objet de l'Operand **/
        ...
        {
            /** Conserve le 1er nom qu'on trouve (si le nom du segment n'a pas été apporté par le File Link) **/
...
static void BuildOneCodeLineOperand(struct source_line *current_line, int *has_error_rtn, struct omf_segment *current_omfsegment, struct omf_project *current_omfproject)
{
    ...
    { 
        ...
        if(is_reloc_1 == 1)
        {
            /* L'Address of the line tient compte des [ORG $Addr ORG] */
        ...
        /** On prépare ce qui va être affiché dans le File Output.txt **/
    ...
    *is_address_rtn = 0;
    ...
    for(i=0; i<nb_element; i++)
    {
        ...
        if(i%2 == 0)
        {
            ...
            {
                /* Error : Impossible de déterminer le format */
        ...
        else
        {
            ...
            /* Error : Impossible de reconnaitre l'opérateur ! */
...
/************************************************************************************************************/
/*  CompactDirectPageCode() : Pass all lines in to review pour détecter les adressages Page Direct. */
/************************************************************************************************************/
